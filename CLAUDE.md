...
## check/ — 抽查单
学完一课／一单元后家长照着问一遍，逐项打勾。`specs/<YYYYMMDD>.txt` → `sheets/<YYYYMMDD>.html|.pdf`
（`--answers` 出家长版）。spec 完整语法见 `check/README.md`。

- **两版分开**：题面版给孩子（答案不印，动笔的题空着写），家长版 `--answers`（答案全印）。
  家长版**不进目录页**，和 practice 的答案版一个道理。
- **以口头题为主**（读音、背诵、复述）——十分钟才问得完。动笔的题超过两三项就该去 `practice/`
  出正经的默写单，别把抽查单撑成练习单。
- `pass=` 要写得能一眼判定（`错 ≤ 1 个`、`零错字`、`三要素齐`），别写"基本掌握"这种没法判的。
  `pass=` 的值可以带空格，所以要写在属性最后。
- 一张单子 5~6 个题块正好一页；题面版下半页的空白由页底错题格（`memo:`）自动吃掉。
- **`specs/20260825.txt` 是早期试做的「跨一~四单元 10 分钟单」，不是范例**——
  抽查要**按每课一张**，这份待改造。
